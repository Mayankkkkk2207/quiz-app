document.addEventListener('DOMContentLoaded', () => {
    ...
    quizQuestionCard.addEventListener('input', (e) => {
        ...
        if (q.type === 'fill-blank' && e.target.name && e.target.name.startsWith('q')) {
            studentAnswers[idx] = e.target.value;
            // Do NOT call renderCurrentQuestion() here
        }
    });
    ...
    quizAttemptForm.onsubmit = async (e) => {
        ...
        try {
            ...
            const token = localStorage.getItem('token');
            const res = await fetch(`/api/quizzes/${currentQuiz._id}/submit`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
    ...
    // --- Student Quiz Scores and Review ---
    const quizScoresSection = document.getElementById('student-quiz-scores');
    const quizReviewModalOverlay = document.getElementById('quiz-review-modal-overlay');
    const quizReviewModal = document.getElementById('quiz-review-modal');
    const closeQuizReviewModalBtn = document.getElementById('close-quiz-review-modal');
    const quizReviewModalTitle = document.getElementById('quiz-review-modal-title');
    const quizReviewContent = document.getElementById('quiz-review-content');

    async function loadStudentQuizSubmissions() {
        if (!user || role !== 'student') return;
        try {
            const res = await fetch('/api/quizzes/my-submissions', {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
            });
            const submissions = await res.json();
            if (!Array.isArray(submissions)) return;
            quizScoresSection.innerHTML = submissions.length === 0 ? '<div class="text-gray-500">No quiz attempts yet.</div>' : submissions.map(sub => `
                <div class="bg-white rounded-xl shadow-md p-6 border border-blue-100 flex flex-col justify-between">
                    <div>
                        <div class="font-bold text-lg text-blue-700 mb-2">${sub.quizTitle}</div>
                        <div class="text-gray-600 mb-2">Score: <span class="font-semibold">${sub.score} / ${sub.total}</span></div>
                        <div class="text-gray-500 text-sm mb-4">Attempted: ${new Date(sub.submittedAt).toLocaleString()}</div>
                    </div>
                    <button class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 mt-auto review-quiz-btn" data-subid="${sub._id}">Review</button>
                </div>
            `).join('');
        } catch (err) {
            quizScoresSection.innerHTML = '<div class="text-red-500">Failed to load quiz scores.</div>';
        }
    }

    quizScoresSection.addEventListener('click', (e) => {
        if (e.target.classList.contains('review-quiz-btn')) {
            const subId = e.target.getAttribute('data-subid');
            showQuizReview(subId);
        }
    });
    ...
    async function showQuizReview(subId) {
        ...
        const sub = lastSubmissions.find(s => s._id === subId);
        if (!sub) return;
        quizReviewModalTitle.textContent = `Quiz Review: ${sub.quizTitle}`;
        quizReviewContent.innerHTML = sub.questions.map((q, idx) => {
            let studentAns = sub.answers[idx];
            let correctAns = '';
            let isCorrect = false;
            if (q.type === 'mcq-single') {
                correctAns = q.options && typeof q.correctAnswer === 'number' ? q.options[q.correctAnswer] : '';
                isCorrect = studentAns === q.correctAnswer;
                studentAns = q.options && typeof studentAns === 'number' ? q.options[studentAns] : '<span class="text-red-500">No answer</span>';
            } else if (q.type === 'mcq-multi') {
                correctAns = q.options && Array.isArray(q.correctAnswers) ? q.correctAnswers.map(i => q.options[i]).join(', ') : '';
                isCorrect = Array.isArray(studentAns) && Array.isArray(q.correctAnswers) && studentAns.length === q.correctAnswers.length && studentAns.every(i => q.correctAnswers.includes(i));
                studentAns = q.options && Array.isArray(studentAns) ? studentAns.map(i => q.options[i]).join(', ') : '<span class="text-red-500">No answer</span>';
            } else if (q.type === 'fill-blank') {
                correctAns = q.correctAnswer;
                isCorrect = (studentAns != null && q.correctAnswer != null && String(studentAns).trim().toLowerCase() === String(q.correctAnswer).trim().toLowerCase());
                studentAns = studentAns || '<span class="text-red-500">No answer</span>';
            }
            return `
                <div class="mb-6 p-4 rounded border ${isCorrect ? 'border-green-300 bg-green-50' : 'border-red-300 bg-red-50'}">
                    <div class="font-semibold mb-2">Q${idx + 1}: ${q.text}</div>
                    <div class="mb-1">Your Answer: <span class="${isCorrect ? 'text-green-700' : 'text-red-700'}">${studentAns}</span> ${isCorrect ? '✔️' : '❌'}</div>
                    <div class="mb-1">Correct Answer: <span class="text-blue-700">${correctAns}</span></div>
                </div>
            `;
        }).join('');
        quizReviewModalOverlay.classList.remove('hidden');
    }

    closeQuizReviewModalBtn && closeQuizReviewModalBtn.addEventListener('click', () => {
        quizReviewModalOverlay.classList.add('hidden');
    });
    quizReviewModalOverlay && quizReviewModalOverlay.addEventListener('click', (e) => {
        if (e.target === quizReviewModalOverlay) quizReviewModalOverlay.classList.add('hidden');
    });
