document.addEventListener('DOMContentLoaded', () => {
        ...
        if (!isTeacher) {
            ...
            (async () => {
                for (const assignment of assignments) {
                    ...
                    let submission = null;
                    try {
                        const res = await fetch(`/assignments/${assignment._id}/mysubmission`, {
                            headers: { 'Authorization': `Bearer ${token}` }
                        });
                        if (res.ok) {
    ...
    if (submitForm && cancelSubmitBtn) {
        ...
        submitForm.onsubmit = async (e) => {
            ...
            try {
                const res = await fetch(`/assignments/${currentAssignmentId}/submit`, {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${token}`
                    },
        ...
        `;
            ...
            submissionsList.querySelectorAll('form[data-feedback-form]').forEach(form => {
                form.onsubmit = async (e) => {
                    ...
                    try {
                        const res = await fetch(`/assignments/submission/${submissionId}/feedback`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'Authorization': `Bearer ${token}`
    ...
    if (editForm && cancelEditBtn) {
        ...
        editForm.onsubmit = async (e) => {
            ...
            try {
                const res = await fetch(`/assignments/${window.currentEditAssignmentId}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${token}`
